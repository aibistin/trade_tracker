...
logging.basicConfig(filename=f"./logs/yfinance_{timestr}.log", level=logging.DEBUG,
...
class YahooFinance:
    ...
    #     logging.info(f"Getting data for {self.stock_symbol}")
    #     try:
    ...
    #     except Exception as e:  # Catch potential exceptions from yfinance
    ...
    def get_stock_data(self, max_age_minutes=60):  # Add max_age_minutes parameter
        ...
        if os.path.exists(file_path) and \
           (time.time() - os.path.getmtime(file_path)) / 60 < max_age_minutes:
