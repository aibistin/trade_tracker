from .models.models import Security, TradeTransaction, get_current_holdings
...
symbols_to_exclude = ['', '14067D508', '14067D607', '873379101',
                      'BMY/R', 'CGRN', 'G06242104', 'MMDA1', 'FAKE1', 'FAKE2', 'FAKE3']
...
@app.route('/index')
def index():
    stmt = select(Security).where(Security.symbol.notin_(
        symbols_to_exclude)).order_by(Security.symbol)
    all_securities = db.session.execute(stmt).scalars().all()

    # Filter out option symbols using regular expressions
    securities = [sec for sec in all_securities if not re.search(
        r'\s+\d{2}/\d{2}/\d{4}\s+\d+\.\d+\s+[A-Z]', sec.symbol)]
...
def view_transaction(transaction_id):
    ...
    return render_template('transaction_detail.html', transaction_id=transaction_id, transaction=transaction)
