...
class TestTradingAnalyzerFilters(unittest.TestCase):
    ...
    def test_status_filter_closed(self):
        ...
        self.assertEqual(
            len(pl_data["stock"]["all_trades"]),
            1,
            f"Expected 1 closed trade, got {len(pl_data['stock']['all_trades'])}",
        )
        self.assertEqual(
            pl_data["stock"]["all_trades"][0].trade_id,
            "0001",
            f"Expected trade ID '0001', got {pl_data['stock']['all_trades'][0].trade_id}",
        )
        self.assertTrue(
            pl_data["stock"]["all_trades"][0].is_done, "Trade should be marked as done"
        )
    ...
    def test_status_filter_open(self):
        ...
        self.assertEqual(
            len(pl_data["stock"]["all_trades"]),
            2,
            f"Expected 2 open trades, got {len(pl_data['stock']['all_trades'])}",
        )
        ...
        self.assertTrue(
            all(not t.is_done for t in pl_data["stock"]["all_trades"]),
