...
class TestTradingAnalyzer(unittest.TestCase):
    ...
    def test_analyze_trades_sn(self):
        ...
        self.assertAlmostEqual(results["SN"]["bought_amount"], -25416.07, places=2)

        self.assertEqual(results["SN"]["sold_quantity"], 235.0)

        self.assertAlmostEqual(results["SN"]["sold_amount"], 20759.77 + 969.9, places=2)
        self.assertEqual(
            results["SN"]["closed_bought_quantity"], 235.0, "SN closed_bought_quantity"
        )
        self.assertAlmostEqual(
            results["SN"]["closed_bought_amount"],
            expected_closed_bought_amount,
            places=2,
        ...
        # Complete Trades
        self.assertEqual(len(results["SN"]["all_trades"]), 5, "SN - all_trades count")
    ...
    def test_analyze_trades_nvda(self):
        ...
        self.assertAlmostEqual(results["NVDA"]["bought_amount"], -60000.0, places=2)
        self.assertEqual(results["NVDA"]["sold_quantity"], 200.0)
        self.assertAlmostEqual(results["NVDA"]["sold_amount"], 62250.0, places=2)
        ...
        self.assertAlmostEqual(results["NVDA"]["open_bought_amount"], 0, places=2)

        self.assertAlmostEqual(results["NVDA"]["profit_loss"], 2250.0, places=2)
        self.assertAlmostEqual(results["NVDA"]["percent_profit_loss"], 3.75, places=2)
        # Complete Trades
    ...
    def test_analyze_trades_tna(self):
        ...
        self.assertAlmostEqual(results["TNA"]["bought_amount"], -7537.5, places=2)
        ...
        self.assertAlmostEqual(results["TNA"]["open_bought_amount"], 0, places=2)

        self.assertAlmostEqual(
            results["TNA"]["profit_loss"], -7537.5 + 4720.0 + 2163.01, places=2
        )
        self.assertAlmostEqual(results["TNA"]["percent_profit_loss"], -8.68, places=2)
        # Complete Trades
    ...
    def test_analyze_trades_nail(self):
        ...
        self.assertEqual(results["NAIL"]["bought_quantity"], expected_bought_qty)
        ...
        self.assertEqual(
            results["NAIL"]["closed_bought_quantity"],
            expected_sold_qty,
            "NAIL closed_bought_quantity",
        )

        self.assertAlmostEqual(
            results["NAIL"]["closed_bought_amount"],
            expected_closed_bought_amount,
            places=2,
        )
        ...
        self.assertAlmostEqual(
            results["NAIL"]["percent_profit_loss"],
            expected_profit_loss_percent,
            places=2,
        )
    ...
    def test_analyze_trades_nio(self):
        ...
        self.assertEqual(results["NIO"]["bought_quantity"], expected_bought_qty)
        ...
        self.assertEqual(
            results["NIO"]["closed_bought_quantity"],
            expected_sold_qty,
            "NIO closed_bought_quantity",
        )

        self.assertAlmostEqual(
            results["NIO"]["closed_bought_amount"],
            expected_closed_bought_amount,
            places=2,
        ...
        self.assertAlmostEqual(
            results["NIO"]["percent_profit_loss"],
            expected_profit_loss_percent,
            places=2,
        )
    ...
    def test_get_open_trades_sn(self):
        analyzer = TradingAnalyzer(self.data_list[0])
        analyzer.analyze_trades()
        open_trades = analyzer.get_open_trades()["SN"]
    ...
    def test_get_open_trades_nvda(self):
        analyzer = TradingAnalyzer(self.data_list[1])
        analyzer.analyze_trades()
        open_trades = analyzer.get_open_trades()["NVDA"]
        expected_open_trades = []
        self.assertEqual(len(open_trades), len(expected_open_trades))
    ...
    def test_get_open_trades_tna(self):
        analyzer = TradingAnalyzer(self.data_list[2])
        analyzer.analyze_trades()
        open_trades = analyzer.get_open_trades()["TNA"]
        expected_open_trades = []
        self.assertEqual(len(open_trades), len(expected_open_trades))
    ...
    def test_get_open_trades_nail(self):
        analyzer = TradingAnalyzer(self.data_list[3])
        analyzer.analyze_trades()
        open_trades = analyzer.get_open_trades()["NAIL"]
    ...
    def test_get_open_trades_nio(self):
        analyzer = TradingAnalyzer(self.data_list[4])
        analyzer.analyze_trades()
        open_trades = analyzer.get_open_trades()["NIO"]
        ...
        for i, expected_trade in enumerate(expected_open_trades):
            ...
            self.assertEqual(got_trade["sells"], expected_trade["sells"])
            self.assertEqual(got_trade["is_done"], expected_trade["is_done"])
            self.assertEqual(got_trade["account"], expected_trade["account"])
